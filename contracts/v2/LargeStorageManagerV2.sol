...
contract LargeStorageManagerV2 {
    ...
    function _putChunkFromCalldata(
        ...
        if (data.length > SLOT_LIMIT) {
            keyToMetadata[key][chunkId] = StorageHelperV2.putRawFromCalldata(data, value).addrToBytes32();
        } else {
    ...
    function _putChunk(
        ...
        // store data and rewrite metadata
        if (data.length > SLOT_LIMIT) {
            keyToMetadata[key][chunkId] = StorageHelperV2.putRaw(data, value).addrToBytes32();
        } else {
            keyToMetadata[key][chunkId] = SlotHelper.putRaw(keyToSlots[key][chunkId], data);
        }
    }

    function _getChunkAddr(bytes32 key, uint256 chunkId) internal view returns (address) {
        bytes32 metadata = keyToMetadata[key][chunkId];
        address addr = metadata.bytes32ToAddr();
        return addr;
    }
